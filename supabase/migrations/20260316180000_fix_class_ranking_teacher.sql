-- ============================================================
-- Fix: incluir professor no ranking da turma
-- Problema: get_class_ranking só consulta class_members (sem professor)
-- Solução: UNION ALL com classes.teacher_id
-- ============================================================
  ...
  EXECUTE format(
    'SELECT COALESCE(jsonb_agg(row_to_json(ranked)), ''[]''::jsonb)
     FROM (
       ...
       FROM (
         -- Alunos
         SELECT user_id FROM public.class_members WHERE class_id = %L
         UNION ALL
         -- Professor
         SELECT teacher_id FROM public.classes WHERE id = %L
       ) all_members
