-- ============================================================
-- Fase 7 — Bloco 2: Streak de Dias Consecutivos
--
-- 1. Reescrever check_daily_missions() para incluir lógica de streak:
--    - Quando primeira missão do dia é completada → atualiza user_streaks
--    - Verifica milestones (7, 14, 30, 60, 100) → grant_xp + baú
-- 2. Retorno de check_daily_missions inclui dados de streak
--
-- Milestones (Visão 8.2):
--   7 dias  → 50 XP
--   14 dias → 100 XP + baú
--   30 dias → 200 XP + baú
--   60 dias → 400 XP + baú
--   100 dias → 1000 XP + baú
-- ============================================================
...
BEGIN
  ...
  -- ============================================================
  -- STREAK: Atualizar quando a primeira missão do dia é completada
  -- ============================================================
  IF v_newly_completed_count > 0 THEN
    -- Buscar streak atual do usuário (com lock)
    SELECT * INTO v_streak
    FROM public.user_streaks
    WHERE user_id = v_user_id
    FOR UPDATE;

    -- Criar registro se não existe (safety net — deveria existir via auth trigger)
    IF NOT FOUND THEN
    ...
    -- Verificar se é a primeira completion do dia (last_active_date != today)
    IF v_streak.last_active_date IS NULL OR v_streak.last_active_date < v_today THEN
      v_is_first_completion_today := true;
      ...
      ELSE
        -- Gap de 1+ dias: resetar para 1
        v_new_streak := 1;
      END IF;

      -- Atualizar user_streaks
      ...
      -- ============================================================
      -- MILESTONES: Verificar e conceder bônus (idempotente)
      -- ============================================================
      FOR v_m IN 1..array_length(v_streak_milestones, 1) LOOP
        IF v_new_streak >= v_streak_milestones[v_m] THEN
          ...
          -- Baú de milestone (idempotente via user_chests UNIQUE)
  ...
  -- Re-fetch streak para retorno (pode ter sido atualizado acima)
