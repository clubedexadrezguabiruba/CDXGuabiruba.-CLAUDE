-- ============================================================
-- Restaurar o XP das missões diárias aos valores especificados
--
-- CONTEXTO (por que esta migration existe):
--   20260316200000_rebalance_xp.sql fez TRÊS mudanças juntas:
--     (a) curva de nível 1.05 -> 1.08
--     (b) XP do pool de missões -35%
--     (c) recálculo único do nível de todos os usuários com 1.08
--   Depois, 20260317100000 e 20260319100000 recolaram corpos antigos de
--   grant_xp e reverteram (a) em silêncio. Produção ficou 4 meses rodando
--   curva 1.05 (rápida) COM missões cortadas em 35% — combinação que nenhum
--   dos dois designs pretendia.
--
-- DECISÃO (2026-07-25): 1.05 é a curva oficial — é o que
--   docs/Recruta64_Visao_do_Produto_v1.md:315 especifica. Logo o corte de 35%,
--   que existia só para compensar 1.08, não tem mais razão de ser.
--   O doc calibra "Dedicado (5 missões/dia) -> ~300 XP/dia"; os valores
--   originais entregam 303 XP/dia, os cortados 208.
--   O 1.08 tornava o nível 100 inalcançável (2.544.775 XP ~ 33 anos).
--
-- COMO ESTA MIGRATION FOI FEITA (importante):
--   o corpo de check_daily_missions abaixo foi extraído de pg_get_functiondef
--   do banco de PRODUÇÃO, e só os 20 valores de reward_xp foram alterados.
--   NÃO foi copiado de uma migration antiga — isso é precisamente o que
--   causou o bug original. Toda a lógica posterior (streak, conquistas,
--   anti-farming, search_path) vem intacta do estado vivo.
--
-- Gate: npm run verify:xp-curve (checa curva E pool contra o documentado)
-- ============================================================
...
-- ------------------------------------------------------------
-- 2. Atualizar as missões já sorteadas para hoje e não concluídas
--    (senão o aluno veria o valor antigo até amanhã)
-- ------------------------------------------------------------
UPDATE public.daily_missions dm
SET reward_xp = v.xp
FROM (VALUES
  ('solve_3_rating', 40), ('solve_5_rating', 50), ('solve_10_rating', 80),
  ('complete_1_lesson', 60), ('complete_2_lessons', 80),
  ('defeat_1_bot', 70), ('defeat_2_bots', 100),
  ('streak_3_puzzles', 50), ('streak_5_puzzles', 70),
  ('do_1_rush', 50), ('do_2_rush', 70),
  ('solve_3_category', 40), ('solve_5_category', 50),
  ('rush_5_correct', 60), ('rush_10_correct', 80),
  ('solve_1_mate2', 50), ('solve_1_fork', 50), ('solve_1_pin', 50),
  ('solve_3_revanche', 60), ('solve_1_endgame', 50)
) AS v(key, xp)
WHERE dm.mission_key = v.key
  AND dm.completed = false;

-- ------------------------------------------------------------
-- 3. Recalcular nível com a curva oficial 1.05
--
--    Necessário porque o passo (c) do rebalanceamento recalculou todos os
--    níveis com 1.08 e nunca foi revertido: os usuários ficaram num estado
--    híbrido (nível derivado de 1.08, XP subsequente concedido por 1.05).
--    Fonte da verdade = soma de xp_grants (mesmo método do rebalanceamento).
-- ------------------------------------------------------------
DO $$
DECLARE
  v_rec record;
...
BEGIN
  FOR v_rec IN SELECT id FROM public.users LOOP
    ...
    LOOP
      v_xp_for_next := round(100 * power(1.05, v_level - 1))::integer;
...
-- Ranking depende de user_public_profiles (matview)
